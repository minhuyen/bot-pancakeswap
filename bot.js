...
const data = {
  ...
  toPurchase: process.env.TO_PURCHASE, // token that you will purchase = BUSD for test '0xe9e7cea3dedca5984780bafc599bd69add087d56'

  amountOfBnb: process.env.AMOUNT_OF_BNB, // how much you want to buy in BNB
...
const tokenOut = data.toPurchase;
...
let buyAction = async () => {
  ...
  try {
    ...
    const amountIn = ethers.utils.parseUnits(`${data.amountOfBnb}`, "ether");
...
app.listen(
  PORT,
  console.log(
    chalk.yellow(`Listening for Liquidity Addition to token ${data.toPurchase}`)
  )
);
